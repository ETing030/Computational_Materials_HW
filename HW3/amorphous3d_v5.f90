...
!------------------------------------------------------------------
          ...
          DO
            ...
            test=(/x,y,z/)
            !WRITE(*,*)"test",test
            IF(((width>=2*radius).AND.(length>=2*radius).AND.(height>=2*radius)).AND.((x==radius.OR.x==width-radius).OR.(y==radius.OR.y==length-radius).OR.(z==radius.OR.z==height-radius)))THEN
              center(:,1)=test
              !WRITE(*,*)center
            ENDIF
            IF(((width>=2*radius).AND.(length>=2*radius).AND.(height>=2*radius)).AND.((x==radius.OR.x==width-radius).OR.(y==radius.OR.y==length-radius).OR.(z==radius.OR.z==height-radius)))EXIT
          ENDDO
